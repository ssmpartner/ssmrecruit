...
export function getAllowedNextStatuses(currentStatus: LeadStatus, isAdmin: boolean): LeadStatus[] {
  ...
  const flowMap: Record<LeadStatus, LeadStatus[]> = {
    ...
    ready_for_controlling: ['controlling_approved', 'rejected'],
...
export const statusConfig: Record<LeadStatus, { label: string; color: string }> = {
  ...
  controlling_approved: { label: 'Controlling freigegeben', color: 'bg-emerald-50 text-emerald-700 border border-emerald-300' },
  management_review: { label: 'GL-Prüfung', color: 'bg-purple-50 text-purple-700 border border-purple-200' },
  management_approved: { label: 'GL freigegeben', color: 'bg-emerald-50 text-emerald-700 border border-emerald-300' },
