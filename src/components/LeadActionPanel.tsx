  ...
  return (
    <div className="space-y-4">
      <StepActionsPanel
        ...
        onOpenDocuments={() => createDocumentRequest('application')}
      ...
      <button
        onClick={() => createDocumentRequest('employment')}
        disabled={sendingDocs}
        className="w-full inline-flex items-center justify-center gap-2 rounded-lg border border-primary/30 bg-primary/5 px-3 py-2 text-xs font-semibold text-primary hover:bg-primary/10 disabled:opacity-50 transition-colors"
        title="Personalstammdaten + Ausweis, Bankkarte, VBV, KK, Führerausweis, Ausländerbewilligung"
      >
        {sendingDocs ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Upload className="h-3.5 w-3.5" />}
        Arbeitsvertrag-Link (Stammdaten + Dossier) erstellen
      </button>

      <button
        onClick={async () => {
          setSendingWelcome(true);
          const { data, error } = await supabase.functions.invoke('send-welcome-email', { body: { lead_id: leadId, auto: false } });
          ...
          if ((data as any)?.skipped) {
            toast({ title: 'Übersprungen', description: `Grund: ${(data as any).skipped}` });
            return;
          }
          toast({ title: '✅ Willkommen-E-Mail gesendet' });
        }}
        disabled={sendingWelcome}
        className="w-full inline-flex items-center justify-center gap-2 rounded-lg border border-emerald-300/50 bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-700 hover:bg-emerald-100 disabled:opacity-50 transition-colors"
        title="Sendet sofort die Willkommen-E-Mail mit Video-Landingpage"
      >
        {sendingWelcome ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Mail className="h-3.5 w-3.5" />}
