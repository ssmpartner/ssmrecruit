              ...
              <div>
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <button className={cn(inputCls, 'flex items-center gap-1.5 text-left mt-1')}>
                      <CalendarIcon className="h-3 w-3 text-muted-foreground" />
                      {contactDate ? format(contactDate, 'dd.MM.yyyy') : 'Wählen'}
                    </button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0" align="start">
                    <Calendar mode="single" selected={contactDate} onSelect={setContactDate} className={cn("p-3 pointer-events-auto")} />
                  </PopoverContent>
                </Popover>
              </div>
              <div>
                <label className="text-xs font-medium text-muted-foreground">Uhrzeit *</label>
                <input type="time" value={contactTime} onChange={e => setContactTime(e.target.value)} className={cn(inputCls, 'mt-1')} />
              </div>
            </div>
            <div>
