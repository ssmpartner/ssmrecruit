...
import { ArrowRight, ArrowDown, AlertTriangle, Clock, TrendingUp, Users, Settings2, Bell, UserCog, ArrowRightLeft, Plus, Trash2, Save, X, ChevronDown, ChevronUp, Eye, Wand2, FlaskConical, LayoutGrid, AlignVerticalJustifyStart, User, Check, Shield, BookOpen, Workflow, FileText, Video, Phone, MapPin, Mail, Upload, Brain, Sparkles } from 'lucide-react';
...
export default function PipelineFlow({ leads, employees = [], onSelectLead }: PipelineFlowProps) {
  ...
  return (
    <div className="rounded-xl border bg-card p-6 shadow-sm space-y-6">
      ...
      {/* Leads in Status Dialog – Enhanced with Process Overview */}
      <Dialog open={viewingStatus !== null} onOpenChange={(open) => !open && setViewingStatus(null)}>
        {viewingStatus && (() => {
          ...
          const stepInfo = PROCESS_STEP_INFO[viewingStatus];
          const statusWizards = wizardLinksMap[viewingStatus] || [];
          const statusRules = escalationRules[viewingStatus] || [];
          const activeStatusRules = statusRules.filter(r => r.enabled);
          const inStatus = activeLeads
            .filter(l => l.status === viewingStatus)
            .map(l => {
              const days = (now - new Date(l.updatedAt || l.createdAt).getTime()) / (1000 * 60 * 60 * 24);
              const activeR = statusRules.filter(r => r.enabled);
          ...
          return (
            <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle className="flex items-center gap-2">
                  <div className={`h-3 w-3 rounded-full ${theme.accent}`} />
                  <span className="text-lg">{EMOJI_MAP[viewingStatus] || '📋'}</span>
                  {stepInfo?.title || statusConfig[viewingStatus].label}
                </DialogTitle>
                {stepInfo && (
              ...
              <Tabs defaultValue="flow" className="mt-3">
                <TabsList className="w-full">
                  <TabsTrigger value="flow" className="flex-1 gap-1.5">
                    <Workflow className="h-3.5 w-3.5" /> Prozess-Flow
                  </TabsTrigger>
                  <TabsTrigger value="leads" className="flex-1 gap-1.5">
                    <Users className="h-3.5 w-3.5" /> Leads ({inStatus.length})
                  </TabsTrigger>
                </TabsList>

                {/* ── Prozess-Flow Tab ── */}
                <TabsContent value="flow" className="space-y-4 mt-3">
                  {/* Visual Flow: Actions → Features → Wizards → Escalation */}
                  <div className="space-y-1">
                    {/* Step 1: Aktionen */}
                    {stepInfo && stepInfo.actions.length > 0 && (
                      <div className="rounded-xl border bg-card p-4 space-y-2.5">
                        <div className="flex items-center gap-2">
                          <div className={`h-6 w-6 rounded-lg ${theme.accent} flex items-center justify-center`}>
                            <span className="text-white text-xs font-bold">1</span>
                          </div>
                          <h4 className="text-sm font-semibold">Aktionen in diesem Schritt</h4>
                        </div>
                        <div className="grid gap-2">
                          {stepInfo.actions.map((action, i) => {
                            const ActionIcon = action.icon;
                            return (
                              <div key={i} className={`flex items-center gap-3 rounded-lg border ${theme.bg} p-3`}>
                                <div className={`shrink-0 h-8 w-8 rounded-lg ${theme.accent}/20 flex items-center justify-center`}>
                                  <ActionIcon className={`h-4 w-4 ${theme.text}`} />
                                </div>
                                <div className="min-w-0">
                                  <p className="text-sm font-medium">{action.label}</p>
                                  <p className="text-xs text-muted-foreground">{action.description}</p>
                                </div>
                                {i < stepInfo.actions.length - 1 && (
                    ...
                    {/* Connector */}
                    {stepInfo && stepInfo.actions.length > 0 && (stepInfo.features.length > 0 || statusWizards.length > 0) && (
                      <div className="flex justify-center py-0.5">
                        <div className={`w-0.5 h-4 ${theme.line} rounded-full`} />
                      </div>
                    )}

                    {/* Step 2: Funktionen */}
                    {stepInfo && stepInfo.features.length > 0 && (
                      <div className="rounded-xl border bg-card p-4 space-y-2.5">
                        <div className="flex items-center gap-2">
                          <div className={`h-6 w-6 rounded-lg ${theme.accent} flex items-center justify-center`}>
                            <span className="text-white text-xs font-bold">2</span>
                          </div>
                          <h4 className="text-sm font-semibold">Funktionen & Automatisierungen</h4>
                        </div>
                        <div className="flex flex-wrap gap-1.5">
                          {stepInfo.features.map((f, i) => (
                    ...
                    {/* Connector */}
                    {(statusWizards.length > 0 || activeStatusRules.length > 0) && (
                      <div className="flex justify-center py-0.5">
                        <div className={`w-0.5 h-4 ${theme.line} rounded-full`} />
                      </div>
                    )}
                      ...
                      <div className="rounded-xl border bg-card p-4 space-y-2.5">
                        <div className="flex items-center gap-2">
                          ...
                          <h4 className="text-sm font-semibold flex items-center gap-1.5">
                            <Wand2 className="h-4 w-4" /> Wizards ({statusWizards.length})
                          </h4>
                        </div>
                        <div className="space-y-2">
                          {statusWizards.map((wiz, i) => (
                            <div key={wiz.id} className="flex items-center gap-3">
                              {/* Order number */}
                              <div className={`shrink-0 h-7 w-7 rounded-full border-2 ${wiz.isActive ? theme.border : 'border-muted'} flex items-center justify-center`}>
                                <span className={`text-xs font-bold ${wiz.isActive ? theme.text : 'text-muted-foreground'}`}>{i + 1}</span>
                              </div>
                              {/* Wizard info */}
                              <div className={`flex-1 rounded-lg border p-2.5 ${wiz.isActive ? theme.bg : 'bg-muted/30 opacity-60'}`}>
                                <div className="flex items-center gap-2">
                                  <p className={`text-sm font-medium ${!wiz.isActive ? 'line-through text-muted-foreground' : ''}`}>{wiz.wizardName}</p>
                                  <span className="rounded-full bg-muted px-1.5 py-0.5 text-[9px] font-semibold text-muted-foreground uppercase">{wiz.wizardType}</span>
                                  {wiz.testOnly && (
                                  ...
                                  {!wiz.isActive && (
                                    <span className="text-[9px] text-muted-foreground italic">Inaktiv</span>
                                  )}
                                </div>
                              </div>
                              {/* Arrow to next */}
                              {i < statusWizards.length - 1 && (
                                <ArrowDown className={`h-4 w-4 ${theme.text} shrink-0`} />
                              )}
                            </div>
                          ))}
                        </div>
                    ...
                    {/* Connector */}
                    {(activeStatusRules.length > 0 || (stepInfo && stepInfo.escalationStatuses.length > 0)) && statusWizards.length > 0 && (
                      <div className="flex justify-center py-0.5">
                        <div className={`w-0.5 h-4 ${theme.line} rounded-full`} />
                      </div>
                    )}
                    ...
                    {/* Step 4: Eskalation */}
                    {((stepInfo && stepInfo.escalationStatuses.length > 0) || activeStatusRules.length > 0) && (
                      <div className="rounded-xl border border-destructive/20 bg-destructive/5 p-4 space-y-2.5">
                        <div className="flex items-center gap-2">
                          <div className="h-6 w-6 rounded-lg bg-destructive flex items-center justify-center">
                            <AlertTriangle className="h-3.5 w-3.5 text-white" />
                          </div>
                          <h4 className="text-sm font-semibold">Eskalation & Regeln</h4>
                        </div>

                        {/* Escalation statuses */}
                        {stepInfo && stepInfo.escalationStatuses.length > 0 && (
                          <div>
                            <p className="text-[11px] font-medium text-muted-foreground mb-1.5">Mögliche Eskalations-Status:</p>
                            <div className="flex flex-wrap gap-1.5">
                              {stepInfo.escalationStatuses.map((es, i) => (
                                <span key={i} className="inline-flex items-center gap-1 rounded-md bg-destructive/10 border border-destructive/20 px-2 py-0.5 text-[11px] font-medium text-destructive">
                                  <Shield className="h-3 w-3" />{es}
                                </span>
                              ))}
                            </div>
                          </div>
                        ...
                        {/* Active rules */}
                        {activeStatusRules.length > 0 && (
                          <div>
                            <p className="text-[11px] font-medium text-muted-foreground mb-1.5">Aktive Eskalationsregeln:</p>
                            <div className="space-y-1.5">
                              {activeStatusRules.map(rule => (
                                <div key={rule.id} className="flex items-center gap-2 rounded-lg border bg-card px-3 py-2">
                                  <Clock className="h-3.5 w-3.5 text-destructive shrink-0" />
                                  <span className="text-xs font-medium flex-1">{rule.name}</span>
                                  <span className="text-[10px] text-muted-foreground">{rule.thresholdDays} Tage</span>
                                  <div className="flex items-center gap-1">
                                    {rule.actions.map((a, ai) => {
                                      const AIcon = ACTION_ICONS[a.type];
                                      return <AIcon key={ai} className="h-3 w-3 text-muted-foreground" />;
                                    })}
                                  </div>
                                  {rule.testOnly && <FlaskConical className="h-3 w-3 text-amber-600" />}
                                </div>
                              ))}
                            </div>
                          </div>
                ...
                <TabsContent value="leads" className="space-y-2 mt-3">
                  {inStatus.length === 0 ? (
                    <div className="text-center py-8 text-muted-foreground">
                      ...
                      <p className="text-sm">Keine Leads in diesem Status</p>
                    </div>
                  ) : (
