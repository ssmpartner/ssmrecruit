...
-- Scope notification recipients to the responsible person for lead-related events.
-- Always include superadmin/admin (oversight). For lead events: include only
-- the assigned employee + agency members (agency_manager/backoffice) of that lead's agency.
-- For non-lead events: keep current rule-matrix behaviour.
...
BEGIN
  ...
  WHERE COALESCE(c.personal_override, c.role_enabled) = true
    AND (
      -- Non-lead event → keep current behaviour
      _lead_id IS NULL
      -- Lead event → scope strictly to responsible persons
      OR c.emp_role IN ('superadmin','admin')
